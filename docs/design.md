...
| 公開                     | 開発版（`APP_CHANNEL=dev`）はこのリポジトリの GitHub Pages に Actions でデプロイして動作確認に使う。一般公開版（`APP_CHANNEL=prod`）は別アカウントのリポジトリの GitHub Pages に置く（整備予定）。どのパスに置かれても動くよう相対パスでビルドする。両者はアプリ名と manifest の `id` を変えてホーム画面上で区別する |
